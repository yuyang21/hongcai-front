  ...
  Dates.events = function() {
    ...
    Dates.each(S('#laydate_table td'),function(i,elem) {
      Dates.on(elem,'click',function(ev) {
        if (!Dates.hasClass(this,as[1])) {
          Dates.stopmp(ev);
          if (Dates.options.format === 'YYYY-MM-DD hh:mm:ss') {
            Dates.viewDate(Dates.ymd[0],Dates.ymd[1],this.getAttribute('d'));
            Dates.creation([this.getAttribute('y') | 0,this.getAttribute('m') | 0,this.getAttribute('d') | 0],1);
          } else {
